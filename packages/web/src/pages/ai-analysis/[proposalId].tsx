...
import { AiAnalysisStandalone } from "../../components/ai-analysis-standalone";

export default function AiAnalysisPage() {
  const [proposalId, setProposalId] = useState<string>("");
  const daoConfig = useDaoConfig();
  const router = useRouter();
  const { proposalId: queryProposalId } = router.query;

  useEffect(() => {
    if (queryProposalId) {
      setProposalId(queryProposalId as string);
    }
  }, [queryProposalId]);
